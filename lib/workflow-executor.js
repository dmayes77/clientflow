 ...
 */
function buildTemplateVariables({ contact, booking, tenant, invoice }) {
  const variables = {};
...
async function executeAction(action, context) {
  ...
  const { contact, booking, tenant } = context;
...
export async function processPendingWorkflows() {
  ...
  for (const run of pendingRuns) {
    ...
    // Fetch required data
    const [contact, booking, tenant] = await Promise.all([
    ...
    const context = { contact, booking, tenant };
