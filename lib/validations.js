...
export const createPackageSchema = z.object({
  ...
  includes: z.array(z.string().max(200)).max(20).optional().default([]), // What's included in this package
});
...
// Note: "inquiry" is a legacy status, kept for backwards compatibility
export const bookingStatusEnum = z.enum(["pending", "inquiry", "scheduled", "confirmed", "completed", "cancelled", "no_show"]);
...
export const invoiceStatusEnum = z.enum(["draft", "sent", "viewed", "paid", "overdue", "cancelled", "void"]);
...
export const createInvoiceSchema = z.object({
  ...
  // Coupon fields
  couponId: z.string().cuid("Invalid coupon ID").optional().nullable(),
  couponDiscountAmount: z.number().int().min(0, "Coupon discount must be 0 or greater").optional().default(0),
...
export const updateInvoiceSchema = createInvoiceSchema.partial().extend({
  // Additional fields allowed only on updates (payment tracking)
  amountPaid: z.number().int().min(0, "Amount paid must be 0 or greater").optional().nullable(),
  balanceDue: z.number().int().min(0, "Balance due must be 0 or greater").optional().nullable(),
  paidAt: z.union([z.string(), z.date(), z.null()]).optional().nullable(),
  sentAt: z.union([z.string(), z.date(), z.null()]).optional().nullable(),
  depositPaidAt: z.union([z.string(), z.date(), z.null()]).optional().nullable(),
});
...
export const updateTenantSchema = z.object({
  ...
  breakStartTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional().nullable(), // HH:MM format
  breakEndTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional().nullable(), // HH:MM format
  bufferTime: z.number().int().min(0).max(60).optional(), // 0-60 minutes buffer
  minAdvanceHours: z.number().int().min(0).max(168).optional(), // 0-168 hours (1 week)
...
export function validateRequest(data, schema) {
  try {
    ...
    if (error instanceof z.ZodError) {
      const errors = error.errors?.map((e) => `${e.path.join(".")}: ${e.message}`) || ["Validation failed"];
