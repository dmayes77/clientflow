...
async function createInvoiceCheckout({ invoiceId, paymentOption }) {
...
async function recordOfflinePayment({ invoiceId, amount, method, notes, isDeposit }) {
  const res = await fetch(`/api/invoices/${invoiceId}/record-payment`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ amount, method, notes, isDeposit }),
  });
  if (!res.ok) {
    const error = await res.json();
...
async function chargeCard({ invoiceId, paymentMethodId, amount, isDeposit }) {
  const res = await fetch(`/api/invoices/${invoiceId}/charge-card`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ paymentMethodId, amount, isDeposit }),
  });
  if (!res.ok) {
    const error = await res.json();
 ...
 * Returns { checkoutUrl, sessionId, amountCharged }
 */
...
/**
 * Record an offline payment (cash, check, venmo, zelle, bank_transfer, other)
 * Returns { success, invoice, payment, isPaidInFull, newBalance }
 */
export function useRecordOfflinePayment() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data.invoice) {
        queryClient.setQueryData([...INVOICES_KEY, variables.invoiceId], data.invoice);
      }
    },
  });
}
...
/**
 * Charge a card using a PaymentMethod from Stripe Elements
 * Returns { success, invoice, payment, paymentIntent, isPaidInFull, newBalance }
 * May return { requiresAction, clientSecret, paymentIntentId } for 3DS
 */
export function useChargeCard() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      // Only invalidate if payment succeeded (not if requires 3DS action)
      if (data.success) {
        queryClient.invalidateQueries({ queryKey: INVOICES_KEY });
        queryClient.invalidateQueries({ queryKey: ["payments"] });
        queryClient.invalidateQueries({ queryKey: ["bookings"] });
