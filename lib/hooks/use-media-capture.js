...
export function useMediaCapture() {
  ...
  const quickCapture = async (options = {}) => {
    ...
    console.log("[useMediaCapture] quickCapture called with options:", options);
    return new Promise((resolve) => {
      const input = document.createElement("input");
      input.type = "file";
      input.accept = "image/*";
      ...
      input.onchange = async (e) => {
        console.log("[useMediaCapture] onchange fired, resolved:", resolved);
        if (resolved) return;
        resolved = true;

        const file = e.target.files[0];
        console.log("[useMediaCapture] File selected:", file);
        if (file) {
      ...
      input.oncancel = () => {
        console.log("[useMediaCapture] oncancel fired, resolved:", resolved);
        if (resolved) return;
        resolved = true;
        resolve({ success: false, cancelled: true });
      };
      ...
      const handleFocus = () => {
        ...
        setTimeout(() => {
          console.log("[useMediaCapture] Focus timeout fired, resolved:", resolved);
          if (!resolved) {
            resolved = true;
            resolve({ success: false, cancelled: true });
          }
      ...
      console.log("[useMediaCapture] Triggering file input click");
      input.click();
    });
  };
