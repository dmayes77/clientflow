...
// Auto-save lead (when entering contact info on welcome screen)
async function autoSaveLead({ slug, name, email, phone }) {
  const res = await fetch(`/api/public/${slug}/lead`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function useAutoSaveLead() {
  return useMutation({
    mutationFn: autoSaveLead,
  });
}
