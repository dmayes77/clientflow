...
import { InvoiceReceiptEmail } from "@/emails/client/invoice-receipt";
 ...
 */
export async function sendEmail({ to, subject, html, from = DEFAULT_FROM, replyTo }) {
  try {
...
/**
 * Build the "from" address for invoice-related emails
 * Uses the tenant's business name in the display name
 */
function buildInvoiceFrom(businessName) {
  const sanitizedName = businessName?.replace(/[<>"]/g, "") || "Your Business";
  return `${sanitizedName} <invoices@getclientflow.app>`;
}

/**
 * Send invoice paid receipt with PDF attachment
 * @param {Object} invoice - The paid invoice object
 * @param {Object} tenant - The tenant object with business info
 */
export async function sendInvoicePaidReceipt(invoice, tenant) {
  const businessName = tenant.businessName || tenant.name || "Business";
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
  const pdfUrl = `${baseUrl}/api/invoices/${invoice.id}/pdf?public=true`;
  ...
  const invoiceWithTenant = {
    ...
    tenant: {
      ...
      businessCity: tenant.businessCity,
      businessState: tenant.businessState,
      businessZip: tenant.businessZip,
      businessPhone: tenant.businessPhone,
    },
  };

  const pdfBuffer = await renderToBuffer(InvoicePDF({ invoice: invoiceWithTenant }));
  ...
  try {
    const result = await resend.emails.send({
      from: buildInvoiceFrom(businessName),
