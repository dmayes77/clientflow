...
import { createDefaultWorkflowsForTenant } from "@/lib/default-workflows";
...
export async function getOrCreateTenant(orgId, userData = {}) {
  ...
  if (!tenant) {
    ...
    try {
      ...
      console.error("Error seeding system templates for tenant:", error);
    }

    // Create default workflows
    try {
      await createDefaultWorkflowsForTenant(prisma, tenant.id);
    } catch (error) {
      console.error("Error creating default workflows for tenant:", error);
    }
  }
