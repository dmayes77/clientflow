 ...
 *
 * This script creates the 9 system email templates for every tenant in the database.
 * System templates are used by cron jobs and automated processes.
 *
...
const SYSTEM_TEMPLATES = {
  ...
  trial_ending: {
    ...
    body: `
      ...
      <p>Hi {{contact.firstName}},</p>
      <p>Your free trial of ClientFlow will end in <strong>3 days</strong>.</p>

      <div style="background-color: #fef3c7; padding: 20px; border-radius: 8px; margin: 24px 0;">
        <p style="margin: 0;">To continue enjoying all features without interruption, please update your billing information before your trial expires.</p>
      </div>

      <p><strong>What happens when your trial ends:</strong></p>
      <ul>
        <li>Access to your account will be restricted</li>
        <li>Your data will be preserved for 30 days</li>
        <li>You can upgrade at any time to restore full access</li>
      </ul>

      <div style="margin: 32px 0;">
        <a href="{{tenant.upgradeUrl}}" style="background-color: #3b82f6; color: white; padding: 14px 28px; text-decoration: none; border-radius: 6px; display: inline-block; font-weight: bold;">Upgrade Now</a>
      </div>

      <p>If you have any questions, we're here to help!</p>
      <p>The ClientFlow Team</p>
    `,
    description: "Sent 3 days before trial expires",
  },

  payment_dispute: {
    name: "Payment Dispute Notification",
    category: "payment",
    subject: "Payment Dispute Filed - Action Required",
    body: `
      <div style="background-color: #fef2f2; border-left: 4px solid #ef4444; padding: 20px; margin-bottom: 24px;">
        <h2 style="margin: 0;">⚠️ Payment Dispute Notification</h2>
      </div>

      <p>A payment dispute has been filed for the following transaction:</p>

      <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 24px 0;">
        <p style="margin: 0;"><strong>Amount:</strong> {{payment.amount}}</p>
        <p style="margin: 12px 0 0 0;"><strong>Payment Date:</strong> {{payment.date}}</p>
        <p style="margin: 12px 0 0 0;"><strong>Payment Method:</strong> {{payment.method}}</p>
        <p style="margin: 12px 0 0 0;"><strong>Transaction ID:</strong> {{payment.confirmationNumber}}</p>
      </div>
      ...
      <ol>
        ...
        <li>Respond to the dispute within the required timeframe</li>
      </ol>

      <p>For assistance with this dispute, please contact our support team immediately.</p>

      <p>ClientFlow Support Team</p>
    `,
    description: "Sent when a payment dispute is filed",
  },
};
