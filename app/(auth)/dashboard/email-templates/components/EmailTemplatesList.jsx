...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/app/(auth)/components/ui/card";
          ...
          <PopoverContent className="w-56 p-2" align="start">
            <div className="grid gap-1 max-h-60 overflow-y-auto">
              {VARIABLES.map((variable) => (
                ...
                >
                  <code className="text-xs bg-muted px-1 rounded mr-2">{variable.value}</code>
                  <span className="et-caption text-muted-foreground">{variable.label}</span>
                </Button>
              ))}
            </div>
          </PopoverContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Create reusable email templates with dynamic variables</CardDescription>
            </div>
          ...
          {filteredTemplates.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filteredTemplates.map((template) => (
                <Card key={template.id} className="py-4 md:py-6 relative group">
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                      <div className="space-y-1 flex-1 min-w-0">
                        <CardTitle className="et-text-base truncate">{template.name}</CardTitle>
                        {template.category && (
                  ...
                  <CardContent>
                    <p className="et-small text-muted-foreground truncate mb-2">
                      <span className="font-medium">Subject:</span> {template.subject}
                    </p>
                    {template.description && <p className="et-caption text-muted-foreground line-clamp-2">{template.description}</p>}
                  </CardContent>
                </Card>
              ))}
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., Welcome to {{business.name}}, {{client.firstName}}!"
                required
              />
              <p className="et-caption text-muted-foreground">You can use variables like {"{{client.name}}"} in the subject line</p>
            </div>
          ...
          {selectedTemplate && (
            <div className="space-y-4">
              <div className="space-y-1">
                <p className="et-caption font-medium text-muted-foreground">Subject</p>
                <p className="et-text-base">{selectedTemplate.subject}</p>
              </div>
              <div className="space-y-1">
                <p className="et-caption font-medium text-muted-foreground">Body</p>
