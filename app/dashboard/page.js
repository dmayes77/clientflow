  ...
  const weeklyBookingData = useMemo(() => {
    ...
    return last7Days.map((date) => {
      const count = bookings.filter((b) =>
        isSameDay(parseISO(b.scheduledAt), date)
      ).length;
    ...
    return (
      <div className="flex h-[500px] items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-xs text-zinc-500">Loading analytics...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          <Button size="sm" className="bg-blue-500 hover:bg-blue-600" onClick={() => router.push("/dashboard/calendar")}>
            <Calendar className="h-3.5 w-3.5 mr-1.5" />
            <span className="hidden sm:inline">New Booking</span>
            <span className="sm:hidden">Booking</span>
          </Button>
          <Button size="sm" variant="outline" className="border-violet-200 text-violet-600 hover:bg-violet-50 hover:text-violet-700" onClick={() => router.push("/dashboard/contacts")}>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Card className="border-l-4 border-l-green-500 bg-linear-to-r from-green-50/50 to-transparent">
          <CardContent className="p-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[0.625rem] font-medium text-green-600 uppercase tracking-wide">
                  Monthly Revenue
                </p>
                <p className="mt-1 text-xl font-bold text-zinc-900">
                  ${monthlyComparison.thisMonthRevenue.toFixed(0)}
                </p>
                <div className="mt-1 flex items-center gap-1">
                  ...
                  <span
                    className={cn(
                      ...
                      monthlyComparison.revenueChange >= 0 ? "text-green-600" : "text-red-600"
                    )}
                  >
                    {monthlyComparison.revenueChange >= 0 ? "+" : ""}
        ...
        <Card className="border-l-4 border-l-blue-500 bg-linear-to-r from-blue-50/50 to-transparent">
          <CardContent className="p-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[0.625rem] font-medium text-blue-600 uppercase tracking-wide">
                  Monthly Bookings
                </p>
                <p className="mt-1 text-xl font-bold text-zinc-900">
                  {monthlyComparison.thisMonthBookings}
                </p>
                <div className="mt-1 flex items-center gap-1">
                  ...
                  <span
                    className={cn(
                      "text-[0.625rem] font-medium",
                      monthlyComparison.bookingsChange >= 0 ? "text-green-600" : "text-red-600"
                    )}
                  >
                    {monthlyComparison.bookingsChange >= 0 ? "+" : ""}
        ...
        <Card className="border-l-4 border-l-violet-500 bg-linear-to-r from-violet-50/50 to-transparent">
          <CardContent className="p-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[0.625rem] font-medium text-violet-600 uppercase tracking-wide">
                  Total Contacts
                </p>
        ...
        <Card className="border-l-4 border-l-amber-500 bg-linear-to-r from-amber-50/50 to-transparent">
          <CardContent className="p-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-[0.625rem] font-medium text-amber-600 uppercase tracking-wide">
                  Active Services
                </p>
                <p className="mt-1 text-xl font-bold text-zinc-900">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2 flex flex-row items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium">Revenue Trend (30 Days)</CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            {topServices.length === 0 ? (
              <div className="flex h-[200px] items-center justify-center">
                <p className="text-xs text-zinc-400">No booking data yet</p>
              </div>
            ) : (
              <div className="space-y-3">
                {topServices.map((service, index) => (
                  <div key={service.name} className="flex items-center gap-3">
                    <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-amber-100 text-[0.625rem] font-bold text-amber-600">
                      {index + 1}
                    </div>
              ...
              <div className="space-y-2">
                {upcomingBookings.map((booking) => (
                  <div
                    ...
                    onClick={() => booking.clientId && router.push(`/dashboard/contacts/${booking.client?.type === "lead" ? "leads" : "clients"}/${booking.clientId}?tab=bookings`)}
                  >
          ...
          <CardContent>
            {recentClients.length === 0 ? (
              <div className="flex h-[180px] flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs text-zinc-400">No contacts yet</p>
                <Button
                  size="sm"
