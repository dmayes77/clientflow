...
export default function SignupLayout({ children }) {
  ...
  return (
    <div
      className="min-h-screen flex flex-col px-4 hig-safe-top hig-safe-bottom bg-linear-to-b from-blue-50 via-purple-50 to-blue-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900"
    >
      {/* Main content */}
      <div className="flex-1 flex flex-col items-center justify-center py-6 fold:py-8 tablet:py-10">
        ...
        <h1 className="hig-large-title tablet:hig-title-1 desktop:hig-large-title text-center mb-6 font-bold tracking-tight">
          <span className="text-slate-800 dark:text-white">Client</span>
        ...
        {!isSuccess && (
          <div className="mb-6 fold:mb-8">
            <StepIndicator currentStep={currentStep} isSuccess={isSuccess} />
          </div>
        )}

        {/* Card - iOS-style with subtle shadow */}
        <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-lg shadow-black/5 dark:shadow-black/20 px-6 py-8 fold:px-8 fold:py-10 tablet:px-10 tablet:py-12">
          {children}
        </div>

        {/* Footer - HIG footnote */}
        <p className="mt-8 hig-footnote tablet:hig-body text-gray-500 dark:text-gray-400 text-center max-w-sm">
